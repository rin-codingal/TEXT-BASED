...
Always be precise, thorough, and educational in your mathematical explanations."""
        ...
        contents = [types.(role="user", parts=[types..from_text(text=full_prompt)])]

        config_params = .GenerateContentConfig(=temperature)
...
def setup_ui():
    st.(page_title="🧮 Math Mastermind", layout="centered")

    st.title("🧮 Math Mastermind")

    st.("**Your Expert Mathematical Problem Solver** - From basic arithmetic to advanced calculus, I'll solve any math problem with detailed step-by-step explanations!")
    
    # Add helpful examples
    with st.("📚 Example Problems I Can Solve"):
        st.markdown("""
    ...
    with st.form(key=, clear_on_submit=True):
        ...
        with :
            difficulty = st.("Level", ["Basic", "Intermediate", "Advanced"], index=1)
