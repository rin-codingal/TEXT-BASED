...
def player_move(board, symbol, player_name):
    move = -1
    while move not in range(,) or not board[,].isdigit():
        try:
            move = int(input(Fore.GREEN + f"{}, enter your move (1-9): "))
            if move not in range(,) or not board[,].isdigit():
                print(Fore. + "Invalid move. Please try again.")
        except ValueError:
            print(Fore. + "Please enter a number between 1 and 9.")
    board[] = symbol
...
def check_full(board):
    return all(not spot.() for spot in board)

def tic_tac_toe():
    print(Fore. + "Welcome to Tic-Tac-Toe!")
    player_name = input(Fore. + "Please enter your name: ")
    ...
    while True:
        ...
        while game_on:
            ...
            if turn == 'Player':
                ...
                if check_win():
                    display_board(board)
                    print(Fore. + f"Congratulations, {player_name}! You have won the game!")
                    game_on = False
                else:
                    if check_full(board):
                        display_board(board)
                        print(Fore. + "It's a tie!")
                        break
                    else:
                        turn = 'AI'
            else:
