...
def generate_response(, temperature=):
    ...
    try:
        contents = [.Content(role="", parts=[types..(text=)])]
        config_params = types.(temperature=temperature)
        response = client..(
...
def run_activity():
    ...
    item = input(f"Enter a specific {} to classify: ")
    
    # Zero-shot example
    print("\n--- ZERO-SHOT LEARNING ---")
    zero_shot = f"Is {} a {}? Answer yes or no."
    print(f"Prompt: {}")
    print(f"Response: {(zero_shot)}")
...
Word: {item}
Story:"""
    print(f"Response: {generate_response(, temperature=)}")
