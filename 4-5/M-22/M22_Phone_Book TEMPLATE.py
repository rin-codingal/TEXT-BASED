		...
		# such as name, number, e-mail id, dob, category etc 
			if j == 0:
				...
				if temp[j] == '' or temp[j] == ' ': 
					...
					sys.("Name is a mandatory field. Process exiting due to blank field...")
			...
			if j == 3:
				...
				if temp[j] == '' or temp[j] == ' ': # Even if this field is left as blank, None will take the blank's place
					temp[j] = None
...
def menu():
	...
	# Menu function will only execute when called
	print("********************************************************************")
	print("\t\t\tSMARTPHONE DIRECTORY", flush=)
	print("********************************************************************")
...
def remove_existing(pb):
	...
	temp = 0 #temp is a checking variable here. We assigned a value 0 to temp.
